...
public class PlayerQuestEntryDataImplementation implements PlayerQuestEntryData {
	...
	public PlayerQuestEntryDataImplementation(PlayerAccountImplementation acc, int questID) {
		this.acc = acc;
	...
	public PlayerQuestEntryDataImplementation(PlayerAccountImplementation acc, int questID, long timer, int finished, int branch, int stage, Map<String, Object> additionalDatas, String questFlow) {
		this.acc = acc;
		this.questID = questID;
		...
		if (branch != -1 && stage == -1)
			QuestsPlugin.getPlugin().getLoggerExpanded().warning("Incorrect quest " + questID + " datas for " + acc.debugName());
	}
	...
	@Override
	public QuestEntryRecord toRecord() {
		return new QuestEntryRecord(questID, finished, timer, branch, stage, additionalDatas, questFlow);
	}
	...
	public static PlayerQuestEntryDataImplementation deserialize(PlayerAccountImplementation acc, Map<String, Object> map) {
		PlayerQuestEntryDataImplementation datas = new PlayerQuestEntryDataImplementation(acc, (int) map.get("questID"));
